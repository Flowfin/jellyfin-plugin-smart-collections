...
if (reports.length === 0) {
  fail(
    `no *.cobertura.xml under ${resultsDir}. The test run collected no coverage, ` +
      "which is not the same as coverage being zero, so this fails rather than passing.",
  );
}
...
for (const [name, score] of [...measured].sort()) {
  ...
  if (!floor) {
    problems.push(
      `${name} is measured and coverage-floors.json declares no floor for it. ` +
        "Add one at the score the suite reaches, so the next change cannot lower it silently.",
    );
  ...
  if (lineShort) {
    problems.push(
      `${name} line coverage is ${fmt(score.line)}% and its floor is ${fmt(floor.line)}%.`,
    );
  }
  if (branchShort) {
    problems.push(
      `${name} branch coverage is ${fmt(score.branch)}% and its floor is ${fmt(floor.branch)}%.`,
    );
  }
  rows.push(
    `  ${name}: line ${fmt(score.line)}% (floor ${fmt(floor.line)}%), ` +
      `branch ${fmt(score.branch)}% (floor ${fmt(floor.branch)}%)` +
      (lineShort || branchShort ? "  BELOW FLOOR" : ""),
  );
}
...
for (const name of Object.keys(declared)) {
  if (!measured.has(name)) {
    problems.push(
      `coverage-floors.json declares a floor for ${name} and no report measured it. ` +
        "Either the assembly is gone and the entry goes with it, or the run stopped covering it.",
    );
  }
}
